...
def generate_blurry_image(img, kernel_parameters, patch_size, ker_size, noise_std):
    ...
    p_blur = np.clip(p_blur + noise_std * noise, 0.0, 1.0) * 255
...
def main(image_paths, n_kernel_per_image, kernel_parameters, patch_size=(400, 400), ker_size=35):
    ...
    add_plot(c2_n_rgb, s2_n_rgb, X_n_rgb**2, Y_n_rgb**2, xlabel='1/f_normal^2',
             ylabel='sigma_0^2', markersize=markersize)
    plt.tight_layout()
    plt.show()

    plt.figure(figsize=(8, 6))
    add_plot(c2_o_rgb, s2_o_rgb, X_o_rgb ** 2, Y_o_rgb ** 2, xlabel='1/f_ortho^2',
             ylabel='sigma_1^2', markersize=markersize)
    plt.tight_layout()
    plt.show()

    plt.figure(figsize=(8, 6))
    add_plot(c_n_rgb, s_n_rgb, X_n_rgb, Y_n_rgb, xlabel='1/f_normal',
             ylabel='sigma_0', do_sqrt=True, markersize=markersize)
    plt.tight_layout()
    plt.savefig('./results/calibration_normal_%2.2f.jpg' % noise_std)
    plt.show()

    plt.figure(figsize=(8, 6))
    add_plot(c_o_rgb, s_o_rgb, X_o_rgb, Y_o_rgb, xlabel='1/f_ortho',
             ylabel='sigma_1', do_sqrt=True, markersize=markersize)
    plt.tight_layout()
...
def optimizeMAE(x, y):
    ...
    res = optimize.linprog(c, A_ub=A, b_ub=b, bounds=bounds, options={'disp': True, 'tol': 1e-8})
...
def add_plot(a, b, x, y, xlabel, ylabel, markersize=2, do_sqrt=False):
    ...
    plt.plot(x, y, 'o', markersize=markersize)
    plt.plot(value_range, regressed_values, color='r')
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.grid(True)
    plt.title('a = %2.3f | b = %2.3f' % (a, b))
